...
export default async function handler(req, res) {
  ...
  try {
    // Buscar por matrícula primero, luego por ID numérico como fallback
    ...
    if (usuRes.rows.length === 0) {
      return res.status(200).json({ success: false, error: 'Estudiante no encontrado' });
    }

    const usuario = usuRes.rows[0];

    // Asistencias confirmadas — si falla el JOIN, devolver lista vacía sin romper
    let asistencias = [];
    try {
      ...
      console.error('Error en query asistencias:', joinErr.message);
    }
    ...
    console.error('Error verificar:', e.message);
